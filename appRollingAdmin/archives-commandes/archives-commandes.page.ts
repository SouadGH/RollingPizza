...
@Component({
  selector: 'app-archives-commandes',
  templateUrl: './archives-commandes.page.html',
  styleUrls: ['./archives-commandes.page.scss'],
})
export class ArchivesCommandesPage implements OnInit {
  allCommandesDeleted: CommandeClient[];
  ...
  getAllCommandesDeleted() {
    this.commandeProvider.getAllCommandesDeleted().subscribe(
      res => {
        if (res['succes'] == false) {
          console.log("Aucune commande payée.");
        } else {
          let sou = <[]>res;
          this.allCommandesDeleted = res;
          console.log("---- Commandes payées : --------");
          console.log(" ->", this.allCommandesDeleted);
 ...
 adresselivraisonCommandesPayees(i){
   alert(JSON.stringify(this.allCommandesDeleted[i].adresseLivraison));
 }
